...
export class QuizTakesService {
  constructor(
    @InjectRepository(QuizTake)
    private readonly takeRepository: Repository<QuizTake>,
  ...
  create(createQuizTakeInput: CreateQuizTakeInput) {
    const created = this.dataSource.transaction(async (manager) => {
      try {
        let z = 0;
        console.log(`---> ${z++}`);
        const quiz = await manager.findOneBy(Quiz, {
          id: createQuizTakeInput.quizId,
        });
        if (!quiz) {
        ...
        for (let i in givenAnswers) {
          ...
          const relatedQuestion = await manager.findOneByOrFail(Question, {
            id: givenAnswers[i].questionId,
          });
          if (!relatedQuestion) {
            throw new NotFoundException(
              `Related question to ${i} given answer not found`,
            );
          } else if (relatedQuestion.quizId != quiz.id) {
            throw new HttpException(
              `Relted question doesn't belong to quiz of id ${quiz.id}`,
              HttpStatus.FORBIDDEN,
            );
          }
          console.log(`---> ${z++}`);
          const relatedAnswers = await manager.find(Answer, {
            where: { questionId: relatedQuestion.id },
          });
          if (!relatedAnswers) {
            throw new NotFoundException(
              `Not found answers to related question ${relatedQuestion.id}`,
            );
          }
          console.log(`---> ${z++}`);
          ...
          switch (relatedQuestion.type) {
            ...
            case QuestionType.SORT_SQUENCE:
              ...
              if (
                givenAnswers[i].sortedAnswers.length < relatedAnswers.length
              ) {
            ...
            default:
              throw new HttpException(
                `Unknown question type: ${relatedQuestion.type}`,
                HttpStatus.FORBIDDEN,
              );
          }
        }

        const take = manager.create(QuizTake, {
          ...createQuizTakeInput,
          score: score,
        });
