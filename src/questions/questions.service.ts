...
import {
  ...
  CreateQuestionInput,
} from './dto/create-question.input';
import {
  UpdateQuestionFullInput,
  UpdateQuestionInput,
...
import {
  CreateAnswerFullInput,
  CreateAnswerInput,
} from 'src/answers/dto/create-answer.input';
  ...
  async create(
    ...
    const newId = await manager.transaction(async (manager) => {
      try {
        ...
        const count = await manager.count(Answer, {
    ...
    const newQuestion = await manager.findOne(Question, {
      relations: ['answers'],
      where: { id: newId },
    });
    if (!newQuestion) {
