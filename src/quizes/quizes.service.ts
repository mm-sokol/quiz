        ...
        for (let questionInput of createQuestionArray) {
          ...
          if (!savedQuestion) {
            throw new BadRequestException(
              `Failed to create question from ${JSON.stringify(questionInput)}`,
            );
          }
          ...
          for (let answerInput of questionInput.answers) {
            ...
            if (!savedAnswer) {
              throw new BadRequestException(
                `Failed to create answer from ${JSON.stringify(answerInput)}`,
              );
            }
          }
        }
    ...
    const quiz = await this.dataSource.transaction(async (manager) => {
      try {
        ...
        if (questions.length >= 0) {
          ...
          const answers = await manager.find(Answer, {
